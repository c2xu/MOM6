...
# include "version_variable.h"
  ...
  if (CS%linear_freq_drag) then
    ...
    if (len_trim(wave_drag_file) > 0) then
      if (CS%use_filter_m2 .and. len_trim(m2_drag_var) > 0 .and. m2_drag_scale > 0.0) then
        allocate(lin_drag_h(isd:ied,jsd:jed), source=0.0)
        ...
        enddo ; enddo
        deallocate(lin_drag_h)
      endif ! CS%use_filter_m2 .and. len_trim(m2_drag_var) > 0 .and. m2_drag_scale > 0.0

      if (CS%use_filter_k1 .and. len_trim(k1_drag_var) > 0 .and. k1_drag_scale > 0.0) then
        allocate(lin_drag_h(isd:ied,jsd:jed), source=0.0)
        ...
        enddo ; enddo
        deallocate(lin_drag_h)
      endif ! CS%use_filter_k1 .and. len_trim(k1_drag_var) > 0 .and. k1_drag_scale > 0.0
    endif ! len_trim(wave_drag_file) > 0
